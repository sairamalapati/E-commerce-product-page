...
mobileMenuBtn.onclick = () => {
    ...
    mobileMenu.classList.contains("active")
        ? mobileMenuBtn.src = "icon-close.svg"
        : mobileMenuBtn.src = "icon-menu.svg";
}
...
sliderImgSelector.forEach(el => {
    el.addEventListener("click", () => {
        sliderImg.src = `image-product-${el.getAttribute("value")}.jpg`;
    });
});
...
const previousImg = (img) => {
    if (counter > 1) {
        counter--;
        img.src = `image-product-${counter}.jpg`;
    } else {
        counter = 4;
        img.src = `image-product-${counter}.jpg`;
    }
...
sliderFocusImgSelector.forEach(el => {
    el.addEventListener("click", () => {
        sliderImgFocus.src = `image-product-${el.getAttribute("focus-value")}.jpg`;
        counter = el.id;
...
removeFromCartBtn.innerHTML = `<img src="icon-delete.svg" alt="">`;
...
const updateCart = () => {
    ...
    let htmlCode = 
        `<img class="cart__product--img" src="image-product-1-thumbnail.jpg" alt="">
